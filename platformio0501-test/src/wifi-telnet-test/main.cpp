...
void setup() {
  Serial.begin(115200);
  MDNS.begin("esp32-telnet-js");  // access this device by esp32-telnet-js.local
    ...
    for (i = 0; i < MAX_SRV_CLIENTS; i++) {
      if (serverClients[i] && serverClients[i].connected()) {
        if (serverClients[i].available()) {
          ...
          Serial.println();
          Serial.print("Telnet len: ");
          Serial.println(serverClients[i].available());
//          while (serverClients[i].available()) Serial.write(serverClients[i].read());
          while (serverClients[i].available()) Serial.println(serverClients[i].read(), HEX);
        }
      }
      else {
        if (serverClients[i]) {
